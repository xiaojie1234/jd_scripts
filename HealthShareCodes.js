...
if (process.env.HEALTHSHARECODES) {
  if (process.env.HEALTHSHARECODES.indexOf('&') > -1) {
    console.log(`您的东东农场互助码选择的是用&隔开\n`)
    HealthShareCodes = process.env.HEALTHSHARECODES.split('&');
  } else if (process.env.HEALTHSHARECODES.indexOf('\n') > -1) {
...
for (let i = 0; i < HealthShareCodes.length; i++) {
  const index = (i + 1 === 1) ? '' : (i + 1);
  exports['HealthShareCode' + index] = HealthShareCodes[i];
}
